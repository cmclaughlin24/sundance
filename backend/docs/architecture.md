...
| **`Tag`**                  | Canonical reference entity scoped to a tenant. Owns one or more `TagVersion` records. Carries a `NodeType` (`primitive` or `object`) describing whether the tag represents a scalar value or a structured object, an optional `PrimitiveType` qualifying the scalar type when `NodeType` is `primitive`, and an `IsCollection` flag indicating whether the tag value is a collection rather than a single value. |
