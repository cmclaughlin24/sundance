...
- **Claim-based outbox relay** — Once a submission is accepted and its canonical facts are produced, a domain event is written to a dedicated `outbox` collection transactionally with the status update. On each tick, the Outbox Relay Worker calls `OutboxRepository.Claim` to atomically transition a batch of eligible events — those with status `pending`, `error`, or `processing` with an expired lease — to `processing` and stamp a `locked_until` timestamp. The worker currently runs with an in-memory elector, meaning all replicas participate concurrently; correctness is enforced entirely by the atomic claim at the database layer rather than by leader election. Stale leases (e.g. from a crashed replica) expire automatically and are reclaimed on the next tick. This reuses the existing `BackgroundWorker[J Job]` pattern rather than introducing MongoDB change streams. See ADR-009.
