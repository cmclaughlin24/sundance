...
- **Claim-based outbox relay** — Once a submission is accepted and its canonical facts are produced, a domain event is written to a dedicated `outbox` collection transactionally with the status update. On each tick, the Outbox Relay Worker calls `OutboxRepository.Claim` to atomically transition a batch of eligible events — those with status `pending`, `error`, or `processing` with an expired lease — to `processing` and stamp a `locked_until` timestamp. All replicas run the worker concurrently; correctness is enforced entirely by the atomic claim at the database layer rather than by leader election. Stale leases (e.g. from a crashed replica) expire automatically and are reclaimed on the next tick. The worker uses `PeriodicWorker[J Job]` — a tick-driven worker with no leader election — rather than `DistributedWorker[J Job]`. See ADR-009.
...
| Message Broker                                              | Outbound  | The Outbox Relay Worker claims batches of eligible events from the `outbox` collection via an atomic lease and publishes each as a canonical submission event |
...
C4Component
  ...
  Container_Boundary(formsService, "Forms Service") {
    Component(restHandlers, "REST Handlers", "chi / Go", "Translates inbound HTTP requests into application commands and queries. Returns JSON responses.")
    Component(appServices, "Application Services", "Go", "Implements FormsAPI, SubmissionsAPI, SubmissionJobsAPI, and TagsAPI. Orchestrates form version lifecycle, idempotent submission intake, and tag version transitions. Delegates the submission processing pipeline to SubmissionProcessor.")
    ...
    Component(processors, "Processors", "Go", "Submission processing pipeline: rule resolution, field validation, and canonical tag normalization. Implements SubmissionProcessor, consumed by both SubmissionJobsAPI and SubmissionsAPI.Normalize.")
...
| **Application Services**       | Implements `FormsAPI`, `SubmissionsAPI`, `SubmissionJobsAPI`, and `TagsAPI`. Orchestrates form version lifecycle, idempotent submission intake, and tag version transitions. Delegates the submission processing pipeline (rule resolution, validation, normalization) to `SubmissionProcessor`.                                                    | `core/services/`        |
| **Domain**                     | `Form`, `FormVersion`, `Submission`, `Tag`, and `TagVersion` aggregates with their value objects (`Page`, `Section`, `Element`, `Rule`, `ElementTagMapping`). Owns all lifecycle state machines (`draft → active → retired`, `draft → active → deprecated → retired`).                                                                                 | `core/domain/`          |
| **Processors**                 | Implements `SubmissionProcessor`. Composes three steps: `submissionResolver` (builds `RuleEvaluationContext`, traverses the form hierarchy, and evaluates `visible`/`required` rules to produce a resolved element list), `submissionValidator` (validates each resolved element via `ElementValidatorRegistry`), and `submissionNormalizer` (resolves winning tag versions and produces `CanonicalFact` entries). Consumed by both `SubmissionJobsAPI` (async, with persistence) and `SubmissionsAPI.Normalize` (synchronous, stateless). | `core/processors/`      |
...
flowchart TD
    ...
    R --> S[StrategyRegistry.Get ElementType]
...
| **`RuleEvaluationContext`**      | `map[string]any` of element key → submitted value. Built once per submission by `submissionResolver` and passed to every rule evaluation call.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `core/ports/secondary.go`                                                |
| **`RuleEvaluator`**              | `ExprRuleEvaluator` evaluates `visible` and `required` rules against the context. Returns `bool`. Determines whether a page, section, or element is processed at all.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `adapters/evaluators/expr_rule_evaluator.go`                             |
...
| **`ElementValidatorStrategy`**     | One implementation per `ElementType` (`text`, `number`, `select`, `checkbox`, `date`). Validates the submitted value against the element's configured constraints.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `core/strategies/`                                                       |
| **Canonical Tag Mapping**        | Two-stage normalization pipeline run after all elements pass validation. **Stage 1 (Tag Version Resolution):** for each `Tag` referenced by the submission's `ElementTagMapping` entries, `domain.ResolveTagVersion` selects the winning `TagVersion` using the Tag Resolution Policy. **Stage 2 (Fact Mapping Priority):** the pipeline dispatches to one of two evaluation paths based on `tag.HasCollectionAncestor()`. For scalar tags, `evaluateScalarCandidates` selects the single highest-priority `ElementTagMapping` winner via `rankCandidates` and produces one `CanonicalFact`. For collection tags, `evaluateCollectionCandidates` groups candidates by `CollectionIndex`, applies `rankCandidates` within each group, and produces one `CanonicalFact` per collection element. Each `CanonicalFact` is keyed by tag key rather than form element key. See Section 6.1 for the Tag Resolution Policy Table and Fact Mapping Policy Table. | `core/processors/submission_normalizer.go`, `core/domain/tag_version.go` |
...
A client submits a form by sending `POST /api/v1/submissions` with an `Idempotency-Key` header. The Forms Service persists the submission with a status of `pending` and immediately returns `202 Accepted` with a reference ID — no validation occurs synchronously. A background worker (leader-elected via Redis) picks up pending submissions, loads the associated form version, and evaluates visibility, required, and read-only rules against the submitted values. For `select` elements backed by a `DataSourceRef`, the Forms Service calls the Tenants Service to resolve the current valid lookup set and validate that the submitted value is a member of it. Note that the Frontend also calls the Tenants Service independently at render time to populate the dropdown options — the validation call at submission time is a server-side confirmation that the selected value remains valid. On success the submission transitions to `accepted`, its canonical facts are stored, and a domain event is written to the `outbox` collection transactionally with the status update. The Outbox Relay Worker — a `PeriodicWorker[J Job]` running on all replicas concurrently with no leader election — calls `OutboxRepository.Claim` on each tick to atomically acquire a batch of eligible outbox events under a short-lived distributed lease and publishes each as a canonical submission event to the message broker, introducing a tick-interval delay between acceptance and delivery. Events whose lease expires before delivery (e.g. due to a replica crash) are automatically reclaimed on the next tick. On validation failure the submission transitions to `rejected`. Failed attempts are retried with exponential backoff up to the configured retry limit.

A synchronous alternative is available via `POST /api/v1/submissions/normalize`. This endpoint runs the same processing pipeline (rule resolution, element validation, and canonical tag mapping) in-memory against the submitted values and returns the resulting fact map immediately — no submission record is created, no status is persisted, and no outbox event is written. This is useful for previewing canonical output before committing a real submission.
...
Once the winning tag version has been selected, the system evaluates all `ElementTagMapping` entries associated with that tag version across the form's elements. The `priority` value on each mapping represents the relative importance of that element as a source for the canonical fact. The mapping with the highest priority wins and its submitted value is used to produce the `CanonicalFact`. If multiple mappings share the same priority, tiebreaker rules are applied to ensure a single deterministic value is selected.
...
| **`FormVersion`**          | Versioned snapshot of a form definition. Composed of a `Page → Section → Element` hierarchy. Follows the `draft → active → retired` lifecycle. Immutable once published.                                                                                                                                                                                                                                                                                                                                                                       |
...
| **`ElementAttributes`**      | Interface with concrete implementations per `ElementType`: `TextElementAttributes` (min/max length, pattern); `NumberElementAttributes` (min/max, step); `SelectElementAttributes` (data source ref, min/max selected); `CheckboxElementAttributes`; `DateElementAttributes` (min/max date).                                                                                                                                                                                                                                                             |
| **`DataSourceRef`**        | Value object on `SelectElementAttributes`. Holds a `DataSourceID` referencing a data source in the Tenants Service and a `Bindings` map for resolving dynamic parameters at render and submission time.                                                                                                                                                                                                                                                                                                                                        |
...
| **`ElementTagMapping`**      | Associates an `Element` with a `TagVersion` at a configurable priority. One element may map to multiple tag versions; the resolution policy selects the winning tag at submission processing time.                                                                                                                                                                                                                                                                                                                                                |
...
| **`Submission`**           | Records a form submission scoped to a tenant, form, and version. Holds `SubmissionValue` entries and progresses through `pending → accepted / rejected / failed`. Identified externally by a `ReferenceID`; deduplicated by `IdempotencyID`. Upon acceptance, the aggregate emits a domain event via the embedded `withEvents` mixin; this event is written to the `outbox` collection transactionally with the status update, enabling the Outbox Relay Worker to deliver the canonical event to the message broker.                   |
| **`SubmissionValue`** | A single submitted value keyed by `ElementID`. Carries an optional `CollectionIndex` — when present, associates the value with a specific element in a collection-backed element, enabling per-element canonical fact production during processing.                                                                                                                                                                                                                                                                                              |
