...
C4Component
  ...
  Rel(processors, fieldValidators, "Resolves validator by FieldType", "StrategyRegistry")
...
| **Processors**                 | Implements `SubmissionProcessor`. Composes three steps: `submissionResolver` (builds `RuleEvaluationContext`, traverses the form hierarchy, and evaluates `visible`/`required` rules to produce a resolved field list), `submissionValidator` (validates each resolved field via `FieldValidatorRegistry`), and `submissionNormalizer` (resolves winning tag versions and produces `CanonicalFact` entries). Consumed by both `SubmissionJobsAPI` (async, with persistence) and `SubmissionsAPI.Normalize` (synchronous, stateless). | `core/processors/`      |
...
| **`RuleEvaluationContext`**      | `map[string]any` of field key → submitted value. Built once per submission by `submissionResolver` and passed to every rule evaluation call.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `core/ports/secondary.go`                                                |
...
| **Canonical Tag Mapping**        | Two-stage normalization pipeline run after all fields pass validation. **Stage 1 (Tag Version Resolution):** for each `Tag` referenced by the submission's `FieldTagMapping` entries, `domain.ResolveTagVersion` selects the winning `TagVersion` using the Tag Resolution Policy. **Stage 2 (Fact Mapping Priority):** the pipeline dispatches to one of two evaluation paths based on `tag.HasCollectionAncestor()`. For scalar tags, `evaluateScalarCandidates` selects the single highest-priority `FieldTagMapping` winner via `rankCandidates` and produces one `CanonicalFact`. For collection tags, `evaluateCollectionCandidates` groups candidates by `CollectionIndex`, applies `rankCandidates` within each group, and produces one `CanonicalFact` per collection element. Each `CanonicalFact` is keyed by tag key rather than form field key. See Section 6.1 for the Tag Resolution Policy Table and Fact Mapping Policy Table. | `core/processors/submission_normalizer.go`, `core/domain/tag_version.go` |
