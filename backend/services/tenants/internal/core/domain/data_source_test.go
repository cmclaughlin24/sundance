...
func TestNewDataSource(t *testing.T) {
	tests := []struct {
		...
		{
			"should create a data lake data source",
			"tenant-1",
			"Galar Pokedex",
		...
		{
			"should yield an error when data lake attributes do not match the type",
			"tenant-1",
			"Galar Pokedex",
			"Generation VIII Pokemon from the Galar region",
			domain.DataSourceTypeDataLake,
...
func TestHydrateDataSource(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"should hydrate a data lake data source",
			"ds-4",
			"tenant-1",
			"Galar Pokedex",
			"Generation VIII Pokemon from the Galar region",
...
func TestDataSource_Update(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"should update a data source to a data lake type",
			newStaticDS(),
			"Galar Pokedex",
			"Generation VIII Pokemon from the Galar region",
			domain.DataSourceTypeDataLake,
			domain.DataLakeDataSourceAttributes{
				Query:      "SELECT value, label FROM pokedex",
				Catalog:    "pokemon",
				Schema:     "galar",
				ValueField: "value",
				LabelField: "label",
